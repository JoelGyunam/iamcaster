			...
			<div class="f-content m-3 rounded">
				<table class="table p-2">
					...
					<c:when test="${empty noticeList }">
						<tr>
							<th class="text-center">아직 공지사항이 없어요.</th>
						</tr>
					</c:when>
					<c:otherwise>
					<c:forEach var="eachNotice" items="${noticeList }">
						<tr>
							<td>&nbsp&nbsp&nbsp<a href="/main/notice/noticeID?id=${eachNotice.noticeID }" class="text-dark"><span>${eachNotice.subject }</span></a></td>
							<td class="text-right"><c:set var="localDate" value="${eachNotice.createdAt.toLocalDate() }" />${localDate }</td>
						</tr>
					</c:forEach>
					</c:otherwise>
					</c:choose>
